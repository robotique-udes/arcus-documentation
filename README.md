# ARCUS

## Arcus repository overview
- [Arcus autonomous driving system overview](code-and-simulation/arcus-repo/overview.md)
- [arcus_master](code-and-simulation/arcus-repo/arcus_master.md)
- [costmap_maker](code-and-simulation/arcus-repo/costmap_maker.md)
- [gap_follow](code-and-simulation/arcus-repo/gap_follow.md)
- [pure_pursuit](code-and-simulation/arcus-repo/pure_pursuit.md)
- [track_zone_manager](code-and-simulation/arcus-repo/track_zone_manager.md)

## Shared Code & Simulation
- [Programming Standards](code-and-simulation/programming-standards.md)
- [SLAM](code-and-simulation/slam.md)
- [EKF](code-and-simulation/ekf.md)
- [Particle Filter](code-and-simulation/particle-filter.md)
- [Launch Car Packages](code-and-simulation/launch_car_packages.md)
- [Arcus VESC car guide](code-and-simulation/arcus-vesc/arcus-vesc.md)

## Environment Setup
- [Installing ROS 2 Humble on Ubuntu 22.04](https://docs.ros.org/en/humble/Installation/Ubuntu-Install-Debs.html)
- [Setting up the containerized environment with f1tenth_gym for simulation](code-and-simulation/setup_up.md)
- [Setting up the SSH key for GitHub](https://docs.github.com/en/authentication/connecting-to-github-with-ssh/generating-a-new-ssh-key-and-adding-it-to-the-ssh-agent)

## Tutorials
- [ARCUS tutorials](tutorials/intro.md)